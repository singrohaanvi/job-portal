...
//           <div className="flex gap-6 lg:gap-8" id="find-jobs-section">
...
const FindJob = () => {
  ...
  // Fetch jobs on filters change (debounced)
  useEffect(() => {
    const timeoutId = setTimeout(() => {
      const apiFilters = {
        keyword: filters.keyword,
      ...
      else fetchJobs(); // fetch all jobs if no filters
    }, 500);
  ...
  const toggleSaveJob = async (jobId, isSaved) => {
    try {
      if (isSaved) await axiosInstance.delete(API_PATHS.JOBS.UNSAVE_JOB(jobId));
      else await axiosInstance.post(API_PATHS.JOBS.SAVE_JOB(jobId));

      toast.success(isSaved ? "Job removed successfully!" : "Job saved successfully!");
      fetchJobs();
    } catch (err) {
      console.log("Error:", err);
      toast.error("Something went wrong! Try again later");
  ...
  // Split applied vs available jobs
  const appliedJobs = jobs.filter((job) => job.applicationStatus);
  const availableJobs = jobs.filter((job) => !job.applicationStatus);
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-4 lg:py-8">
          ...
          <div className="flex gap-6 lg:gap-8" id="find-jobs-section">
            ...
            <div className="flex-1 min-w-0">
              ...
              {/* Available Jobs */}
              <div>
                <h2 className="text-2xl font-bold mb-6">Find Your Next Job</h2>
                {availableJobs.length === 0 ? (
                  <p className="text-gray-600">No more jobs available at the moment.</p>
                ) : (
                  <div className={viewMode === "grid" ? "grid grid-cols-1 lg:grid-cols-2 gap-4" : "space-y-4"}>
