        ...
        >
            <div className="flex items-start justify-between mb-4">
                ...
                {user && (
                    ...
                    >
                        <Bookmark
                            className={`w-5 h-5 hover:text-primary ${job?.isSaved || saved ? "text-primary" : "text-gray-400"
                                }`}
                        />
                    </button>
                )}
            ...
            <div className="flex items-center justify-between">
                <div className="text-primary font-semibold text-lg">
                    {formatSalary(job?.salaryMin, job?.salaxyMax)}
                </div>
                {!saved && (
                    <>
                        {job?.applicationStatus ? (
                            ...
                            !hideApply && (
                                <button
                                    className="bg-gradient-to-r from-primary/10 to-primary/10 text-primary hover:text-white px-6 py-2.5 rounded-xl hover:from-primary hover:to-secondary transition-all duration-200 font-semibold transform hover:-translate-y-0.5"
                                    onClick={(e) => {
                                        e.stopPropagation();
                                        onApply();
                                    }}
